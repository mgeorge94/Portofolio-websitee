...
const curriculumChapterTitle = curriculumChapter.querySelector('.chapter-title');
const experienceChapterTitle = experienceChapter.querySelector('.chapter-title');
...
const autoWriteText = (chapter) => {
 ...
 paragraphsArr.forEach((paragraph) => {
  if (paragraph.name === paragraphHTML.dataset.chapter) {
   ...
   //  if (scrollable === true) {
   //   setTimeout(() => {
   //    paragraphHTML.scrollIntoView({ behavior: 'smooth' }, 700);
   //   }, 400);
   //  }
...
(function hideCertificates() {
 ...
 const closeCertificationsBtn = certificateContainer.querySelector('.fa-angle-right');
 const projectsContainer = document.querySelector('.experience-chapter');
...
const animateProjectCard = () => {
 const carouselCountainer = document.querySelector('.projects-carousel-container');
 const card = document.querySelector('.project-card');
 const projectsContainer = document.querySelector('.projects-carousel-container');
 projectsContainer.addEventListener('mousemove', (e) => {
  let xAxis = (carouselCountainer.getBoundingClientRect().width * 0.9 - e.pageX) / 25;
  let yAxis = (carouselCountainer.getBoundingClientRect().height * 3.5 - e.pageY) / 15;
...
const insertImagesInSlideshow = () => {
 ...
 moreProjectImages.forEach((image) => {
  ...
  imageSlideshowContainerHTML.insertAdjacentHTML('beforeend', imageSlideshowHTML);
 });
...
const clickOnProjectDetailsImage = (imagesGrid) => {
 ...
 images.forEach((image) => {
  image.addEventListener('click', () => {
   const slideshow = (document.querySelector('.slideshow-container').style.display = 'block');
...
const insertProjectDetails = (project) => {
 const projectDetailsContainer = document.querySelector('.project-details-container');
 const projectDetailsPictureContainer = document.querySelector('.project-details-picture-grid');
...
function showProjectDetails(seeMoreBtn) {
 ...
 let projectDetailsContainer = document.querySelector('.project-details-container');
 let experienceChapterContainer = document.querySelector('.experience-chapter');
